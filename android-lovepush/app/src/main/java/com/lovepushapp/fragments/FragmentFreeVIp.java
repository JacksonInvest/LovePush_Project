...
public class FragmentFreeVIp extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        txtType.setText("$" + plansEntity.getData().get(2).getPrice());
        rvPlan.setLayoutManager(new LinearLayoutManager(getContext(), RecyclerView.VERTICAL, false));
        rvPlan.setAdapter(new PlansAdapter(plansEntity.getData().get(2)));

        if (profile.getData().getPlan_details() != null) {
            if (profile.getData().getPlan_details().getPlanId() == plansEntity.getData().get(2).getId()) {
        ...
        txtAction.setOnClickListener(v -> {
            if (txtAction.getText().toString().equalsIgnoreCase("Subscribe")) {
                ...
                intent.putExtra("id", plansEntity.getData().get(2).getPlanId());
