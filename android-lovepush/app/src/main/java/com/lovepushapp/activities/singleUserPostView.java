...
public class singleUserPostView extends BaseActivity  implements PostAdsMvp {
    ...
    private Activity context = singleUserPostView.this;
        ...
        adsPresenter.hitDeleteMyHomePost(data.getId(), new ResponseListner() {
            ...
            public <T> void onComplete(Response<T> response) {
                if (response.body() instanceof postAdDeleteResponse) {
                    postAdDeleteResponse rees = (postAdDeleteResponse) response.body();
//            appUtils.showToast(response.getMessage());
                    if (rees.getStatus() == API_GLOBALS.RESPONSE_CODE.SUCCESS) {
                        appUtils.showToast(rees.getMessage());
                        goback();
                    }
                    else
                    {
