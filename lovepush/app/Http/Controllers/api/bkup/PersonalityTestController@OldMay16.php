...
{   
    ...
    public function userAnswerSave(Request $request) {
        ...
        if($request->question_id == 18) {
            ...
            $res = $this->_filterResult($res['scores']);
            echo "<pre>"; print_r($res); die;
    ...
    public function _filterResult($res) {
        ...
        foreach ($res as $que_id => $anss) {
            if($que_id == 5){
                ...
                //get whose values available 
                foreach($anss as $key  => $ans){

                    $rule_exixts = $this->_checkRuleExists($ans['my_ans'], $ans['other_ans_id']);
                    

                    // echo 'my_ans = ' . $ans['my_ans'].'<br>';
                    // echo 'other_ans_id = ' . $ans['other_ans_id'].'<br>';
                    // var_dump($rule_exixts);
                    // echo "<pre>before"; print_r($anss); 
                    ...
                    if($rule_exixts == true){
                        ...
                        // $new_arr = array_merge($new_arr,$rem_arr);

                        // $anss = $new_arr;
                                            
                        // echo "<pre>after "; print_r($new_arr); die;
    ...
    function _checkRuleExists($ans_id,$other_ans_id){
        if( ($ans_id == 17) && ($other_ans_id == 21) ) {
            return true;
        } else if( ($ans_id == 21) && ($other_ans_id == 17) ) {
            return true;
