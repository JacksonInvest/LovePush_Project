...
{   
    ...
    public function userAnswerSave(Request $request) {
        ...
        if($request->question_id == 18) {
            ...
            foreach ($tests as $key => $test) { // select one test 

                $o_tests = $this->_getOtherTests($tests,$test['id']);
                // echo "<pre> o_tests = "; print_r($o_tests); 

                foreach ($test['ques'] as $key => $c_que) { //select one que
                    ...
                    foreach ($o_tests as $key => $o_test) {
                        foreach ($o_test['ques'] as $key => $o_que) {
                            ...
                            if($c_que['ques_id'] == $o_que['ques_id'] ){     ////now two ques matched
                                ...
                                //now compare answers
                                $scores = $this->_getScores($que_id, $c_que['answers'], $o_que['answers'], $test['id'], $o_test['id']);
                ...
                //remove first key from score
                $new_res = [];
                foreach($res['scores'] as $test1_ids){
                    // $new_res[test1_id][test2_id][i][que_id] = comp_res[]
                        ...
                        foreach($test2_ids as $test2_id => $que_ids){
                            foreach($que_ids as $que_id => $comparisons){
                                foreach($comparisons as $k => $value){
                                    ...
                                    // echo "<pre>value = "; print_r($value); die; // die; //for one user

                                    $new_res['test1_id '.$test1_id]['test2_id '.$test2_id]['que_id '.$que_id][] = $value;
                ...
                // echo "<pre>last res = "; print_r($res); die; // die; //for one user
                echo "<pre>new res = "; print_r($new_res); die; // die; //for one user
                //Save scores 
    ...
    public function _getOtherTests($tests,$ignore_test_id){
        return Collect($tests)->reject(function($test) use ($ignore_test_id){
            return $test['user_id'] === $ignore_test_id;
        });
    }
