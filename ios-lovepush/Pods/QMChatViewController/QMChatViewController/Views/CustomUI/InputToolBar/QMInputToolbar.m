...
- (void)leftBarEmojiButtonPressed:(UIButton *)sender {
...
- (void)rightBarMikeButtonPressed:(UIButton *)sender {
    
//    [self.delegate messagesInputToolbar:self didPressRightBarMikeButton:sender];
}
...
//    }
    ...
    buttonToUpdate = self.contentView.rightBarMikeButtonItem;
    buttonContainer = self.contentView.rightBarButtonContainerView;
...
//        [self addCenterConstraintsToItem:buttonContainer];
    ...
    self.audioRecordButtonItem.hidden = false;//hasDataToSend;
}
...
//    if (self.sendButtonOnRight) {
//
//        self.contentView.rightBarMikeButtonItem.enabled = hasText || hasTextAttachment;
//    }
//    else {
//
//        self.contentView.rightBarMikeButtonItem.hidden = !(hasText || hasTextAttachment);
//    }
}
        ...
        if (object == self.contentView) {
            ...
            if ([keyPath isEqualToString:NSStringFromSelector(@selector(leftBarButtonItem))]) {
                ...
                [self.contentView.rightBarButtonItem addTarget:self
                                              ...
                                              forControlEvents:UIControlEventTouchUpInside];
            }else if ([keyPath isEqualToString:NSStringFromSelector(@selector(rightBarMikeButtonItem))]) {
                
                [self.contentView.rightBarMikeButtonItem removeTarget:self
                                                           action:NULL
                                                 forControlEvents:UIControlEventTouchUpInside];
                
                [self.contentView.rightBarMikeButtonItem addTarget:self
                                                        action:@selector(rightBarMikeButtonPressed:)
                                              forControlEvents:UIControlEventTouchUpInside];
            }
...
- (void)addObservers {
    ...
    [self.contentView addObserver:self
                       forKeyPath:NSStringFromSelector(@selector(rightBarButtonItem))
                          ...
                          context:kQMInputToolbarKeyValueObservingContext];
    [self.contentView addObserver:self
                       forKeyPath:NSStringFromSelector(@selector(rightBarMikeButtonItem))
                          options:0
                          context:kQMInputToolbarKeyValueObservingContext];
...
- (QMAudioRecordButton *)audioRecordButtonItem {
    ...
    if (!_audioRecordButtonItem) {
        ...
        UIImage *recordImage = [QMChatResources imageNamed:@"mike_ic"];
