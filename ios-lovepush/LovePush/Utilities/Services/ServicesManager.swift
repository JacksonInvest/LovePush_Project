...
class ServicesManager: QMServicesManager {
    ...
    func subscribeQBNotification() {
        let deviceIdentifier = UIDevice.current.identifierForVendor?.uuidString
        
        let subscription = QBMSubscription()
        subscription.notificationChannel = QBMNotificationChannel.APNS
        subscription.deviceUDID = deviceIdentifier
        subscription.deviceToken = Singleton.shared.deviceToken
//        print(Singleton.shared.deviceToken)
    ...
    func handleNewMessage(message: QBChatMessage, dialogID: String) {
        ...
        var dialogName = "SA_STR_NEW_MESSAGE"
        ...
        if dialog.type != QBChatDialogType.private {
            ...
            print(dialog.recipientID)
            if let user = ServicesManager.instance().usersService.usersMemoryStorage.user(withID: UInt(dialog.recipientID)) {
                dialogName = user.login!
            }
        }
//        QMMessageNotificationManager.showNotification(withTitle: dialogName, subtitle: message.text, type: QBMNotificationType.info)
    }
