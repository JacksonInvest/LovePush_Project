...
public class RKPieChartView: UIView {
    ...
    public var centerTitleSize : CGFloat = 8.0 {
        didSet {
            setNeedsLayout()
        }
    }
        ...
        items.forEach { (item) in
            ...
            if(!isAnimationActivated) {
                ...
                if let lineCap = (LineCapStyle(rawValue: Int(style.rawValue))?.description) {
                    shapeLayer.lineCap = CAShapeLayerLineCap(string: lineCap) as String//CAShapeLayerLineCap(rawValue: lineCap)
                }
            ...
            if(centerLabel == nil && centerTitle != nil) {
                ...
                centerLabel?.font = UIFont(name: "HelveticaNeue", size: self.centerTitleSize)
                centerLabel?.minimumScaleFactor = 0.7
                centerLabel?.numberOfLines = 2
                centerLabel?.textAlignment = .center
                centerLabel?.textColor = centerTitleColor//UIColor.white
